...
namespace UnityEngine.Perception.GroundTruth
{
    ...
    /// </summary>
    [Serializable]
    public sealed class CustomKeypointLabeler : CameraLabeler
    {
        internal const float defaultSelfOcclusionDistance = 0.15f;
        const int k_MinTextureWidth = 4;

        static ProfilerMarker k_OnEndRenderingMarker = new ProfilerMarker($"CustomKeypointLabeler OnEndRendering");
        static ProfilerMarker k_OnVisualizeMarker = new ProfilerMarker($"CustomKeypointLabeler OnVisualize");
        ...
        public CustomKeypointLabeler(IdLabelConfig config, KeypointTemplate template)
        {
        ...
        protected override void Setup()
        {
            if (idLabelConfig == null)
                throw new InvalidOperationException($"{nameof(CustomKeypointLabeler)}'s idLabelConfig field must be assigned");
